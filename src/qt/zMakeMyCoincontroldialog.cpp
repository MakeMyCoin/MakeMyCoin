...
#include "ui_zMakeMyCoincontroldialog.h"
...
std::list<std::string> ZMakeMyCoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZMakeMyCoinControlDialog::listMints;

ZMakeMyCoinControlDialog::ZMakeMyCoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMakeMyCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZMakeMyCoinControlDialog::~ZMakeMyCoinControlDialog()
{
    delete ui;
}

void ZMakeMyCoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMakeMyCoinControlDialog::updateList()
{
...
void ZMakeMyCoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMakeMyCoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZMakeMyCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMakeMyCoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZMakeMyCoinControlDialog::GetSelectedMints()
{
...
void ZMakeMyCoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
