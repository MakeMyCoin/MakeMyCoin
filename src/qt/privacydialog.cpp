...
#include "zMakeMyCoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zmmco ought to be enough for anybody." - Bill Gates, 2017
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        ui->pushButtonMintzsnd->setToolTip(tr("zmmco is currently disabled due to maintenance."));

        ui->pushButtonSpendzsnd->setEnabled(false);
        ui->pushButtonSpendzsnd->setToolTip(tr("zmmco is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_pushButtonMintzsnd_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zmmco is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zmmco...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zmmco in ") +
...
void PrivacyDialog::on_pushButtonSpendzsnd_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zmmco is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zmmco
        sendzsnd();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zmmco
    sendzsnd();
}

void PrivacyDialog::on_pushButtonZMakeMyCoinControl_clicked()
{
    ZMakeMyCoinControlDialog* zsndControl = new ZMakeMyCoinControlDialog(this);
    zsndControl->setModel(walletModel);
    zsndControl->exec();
}

void PrivacyDialog::setZMakeMyCoinControlLabels(int64_t nAmount, int nQuantity)
{
...
void PrivacyDialog::sendzsnd()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid MakeMyCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    // Warn for additional fees if amount is not an integer and change as zmmco is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MakeMyCoin </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zmmco</b>";
    ...
    // use mints from zmmco selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZMakeMyCoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZMakeMyCoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zMakeMyCoin selector in case it was used
    ZMakeMyCoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zmmco Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " MakeMyCoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zmmco Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zmmco </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zmmco "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zmmco "));
