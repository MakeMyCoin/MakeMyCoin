...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 MakeMyCoin at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zmmco send
enum ZerocoinSpendStatus {
    ZMakeMyCoin_SPEND_OKAY = 0,                            // No error
    ZMakeMyCoin_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMakeMyCoin_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMakeMyCoin_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMakeMyCoin_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMakeMyCoin_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMakeMyCoin_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMakeMyCoin_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMakeMyCoin_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMakeMyCoin_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMakeMyCoin_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMakeMyCoin_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMakeMyCoin_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMakeMyCoin_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMakeMyCoin_SPENT_USED_ZMakeMyCoin = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZMakeMyCoinAutoBackups(bool fEnabled)
    {
